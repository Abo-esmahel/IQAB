<!DOCTYPE html><html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}" class="h-full scroll-smooth">
<head>
    <meta charset="UTF-8">
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    fontFamily: {
                        sans: ['Cairo', '"Instrument Sans"', 'ui-sans-serif', 'system-ui', 'sans-serif'],
                    },
                    colors: {
    ...
    <style>
        ...
        .text-gold-gradient {
            background: linear-gradient(120deg, #FFF9D2 0%, #DFBA54 35%, #C3922E 70%, #FF9900 100%);
...
<body class="min-h-full bg-dark-950 text-dark-100 font-sans antialiased">
    ...
    {{-- Opening splash: شاشة الاستقبال الفاخرة لمدة 5 ثوانٍ بدون طائر وبدون ريش --}}
    <div id="iqab-splash" class="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-dark-950 transition-opacity duration-700 p-6 text-center">
        <div aria-hidden="true" class="pointer-events-none absolute inset-0 overflow-hidden">
            <div class="absolute -top-40 start-1/2 -translate-x-1/2 h-[36rem] w-[80rem] rounded-full bg-[radial-gradient(closest-side,rgba(216,156,43,0.15),transparent)]"></div>
        ...
        <button id="iqab-splash-skip" type="button"
                class="absolute top-5 end-5 z-10 text-xs font-semibold px-3 py-1.5 rounded-lg border border-dark-700 bg-dark-900/60 text-dark-400 hover:text-primary-400 hover:border-primary-500/40 transition-colors flex items-center gap-1.5">
        ...
        <div class="max-w-2xl w-full flex flex-col items-center">
            <!-- اسم عقاب بالخط الذهبي الملكي النقي بدون طائر وبدون ريشات -->
            <h1 dir="rtl" class="text-6xl sm:text-7xl font-black text-gold-gradient tracking-wider drop-shadow-[0_0_35px_rgba(216,156,43,0.45)] mb-4">
                عُـقـاب
            </h1>


            <!-- الشعار المزدوج -->
            <div dir="rtl" class="text-xl sm:text-2xl font-black mb-6 flex items-center justify-center gap-2 flex-wrap">
                <span class="text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.25)]">رؤيةٌ لا تُخطئُ،</span>
                <span class="text-gold-gradient">ومخالبُ لا تُفوّتُ.</span>
            </div>


            <!-- النص التعريفي السيبراني الكامل كما في الصورة -->
            <div dir="rtl" class="text-sm sm:text-base text-dark-300 leading-relaxed max-w-xl space-y-3 mb-8">
                <p>في عالمٍ رقميٍّ مليءٍ بالمتغيرات، تأتي منصة <strong class="text-primary-400 font-bold">عُقاب</strong> لتكون عينك الصقرية التي تراقب وتحمي وتنمي.</p>
                <p>نحن لسنا مجرد منصة خدمات؛ نحن ذراعك السيبرانية الطويلة، نوصلك إلى ما تبحث عنه بسرعة الضوء، وبسرية تامة تخترق بها ضجيج المنافسين.</p>
                <p class="text-primary-400 font-bold text-base mt-2">مع عُقاب، أنت دائماً في القمة.</p>
            </div>


            <!-- مؤشر التحميل الدوار وشريط التقدم للـ 5 ثوانٍ -->
            <div class="flex flex-col items-center gap-3">
                <div class="w-10 h-10 border-[3px] border-dark-800 border-t-primary-500 border-r-primary-400 rounded-full animate-spin shadow-[0_0_20px_rgba(216,156,43,0.35)]"></div>
                <div class="h-1 w-56 sm:w-64 overflow-hidden rounded-full bg-dark-800">
                    <div id="iqab-splash-progress" class="h-full rounded-full bg-gradient-to-r from-primary-600 via-primary-500 to-primary-300" style="animation: iqab-progress 5s linear forwards;"></div>
                </div>
                <span class="text-[11px] text-dark-500 tracking-wider">جاري إطلاق المنصة الملكية...</span>
            </div>
        </div>
    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                $features = [
                    ...
                    ['icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>', 'title' => 'SMS Inbox', 'desc' => 'Receive and view SMS messages in real-time with a clean, organized inbox.'],
                    ['icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/>', 'title' => 'Telegram Tools', 'desc' => 'Access Telegram lookup, report, and information services.'],
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="relative grid sm:grid-cols-3 gap-10">
                ...
                <div class="reveal relative text-center px-4" style="transition-delay:{{ $index * 90 }}ms">
                    <div class="relative z-10 mx-auto h-14 w-14 rounded-full bg-dark-950 border border-primary-500/50 flex items-center justify-center glow-ring shadow-xl shadow-black">
    ...
    {{-- سكريبت شاشة الاستقبال: 5 ثوانٍ فقط ثم الاختفاء --}}
    <script>
        (function () {
            ...
            // مؤقت الإغلاق التلقائي بعد 5 ثوانٍ بالضبط
