    ...
    <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 lg:hidden">
                ...
                <span class="text-lg font-bold bg-gradient-to-r from-primary-300 via-primary-400 to-primary-600 bg-clip-text text-transparent">IQAB</span>
            </a>
        </div>
            ...
            @auth
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-cloak x-transition x-cloak @click.away="open = false"
                         class="absolute end-0 mt-2 w-56 rounded-xl bg-dark-800 border border-dark-700 shadow-xl py-2 z-50">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full text-start px-4 py-2 text-sm text-red-400 hover:bg-dark-700">Logout</button>
                        </form>
                    </div>
                </div>
            @endauth
